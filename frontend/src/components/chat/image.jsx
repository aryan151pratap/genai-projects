...
export default function Image_show({image, setImage}){
		...
		<div className="flex flex-wrap justify-end gap-2">
			{image.map((i, index) => (
				<div key={index} className="border border-white/0 hover:border-white/20 rounded group flex flex-row gap-2 relative w-fit bg-white/10 hover:bg-white/15 transition duration-300 p-1">
					<div className="transition-all duration-500 flex flex-row gap-2 overflow-auto">
						{i.image ?
							<img src={i?.image} className="p-0.5 border border-white/20 rounded max-w-15 h-10 object-cover" />
							:
							<div className="shrink-0 p-1 px-2 text-xs flex items-center bg-black/20 border border-white/10 rounded-md">
								{i?.name.split(".")[1]}
							</div>
						}
						<div className="max-w-40 shrink-0 text-xs flex flex-col justify-center">

...
function Bot({chatId, user_data}){
	...
	const handleSubmit = async function(){
		...
		try{
			...
			setOutput(e => [...e, {role: user.role, content: {text: input, image: image}}]);
			setInput("");
							...
							<div className={`relative ${down ? "w-full" : "w-fit"} flex flex-row items-center justify-center gap-2`}>
								<div className="w-full items-center flex flex-col">
									...
									<div className={`w-full ${role ? "rounded-tl" : ""} flex flex-col items-center rounded-2xl transition duration-300 border-white/20 border-1 focus-within:border-white/60 bg-black/80 text-white overflow-auto`}>
										...
										{image.length > 0 &&
											<div className="w-full max-h-50 overflow-auto p-2">
												<Image_show image={image} setImage={setImage}/>
											</div>
										}
								...
								<button className={`p-2 h-fit w-fit text-black rounded-lg shadow-md shadow-black/50 hover:shadow-black ${loading ? "bg-white/70 cursor-not-allowed" : "bg-white/90 hover:bg-white/80 focus:bg-white/60 cursor-pointer"}`}
